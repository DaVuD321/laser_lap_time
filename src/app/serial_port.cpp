...
SerialPort::SerialPort(std::string port_name, unsigned int baud_rate,
unsigned int reading_buffer_size)
{
  serial_state = 0;

  //serial_port_fd = open( port_name.c_str(), O_RDWR | O_NOCTTY | O_NODELAY);
  ...
  reading_thread = new std::thread(&SerialPort::serial_read_thread,
this);
}
...
void SerialPort::put_string(std::string str)
{
